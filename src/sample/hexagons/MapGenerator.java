...
class MapGenerator {
    ...
    MapGenerator(HexagonMap map, int mapHeightInHexes, int mapWidthInHexes){
        this.map = map;
        this.mapHeight = mapHeightInHexes;
        this.mapWidth = mapWidthInHexes;
        this.image = new Image("https://i.imgur.com/6Ys3fRp.png");
    }
    ...
    // custom generate not from image
    public void generate(IHexagonCreatorWithoutColor creator) {
        ...
        for (int x = 0; x < mapWidth; x++) {
            for (int y = 0; y < mapHeight; y++) {
                ...
                Hexagon h = new Hexagon(axialQ, axialR);
                h.setMap(map);
                creator.createHexagon(axialQ, axialR, map);
            }
        }
    }
}
